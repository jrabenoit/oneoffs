...
# ECC FOR 5 T2'S USING AVERAGE T2 AS REFERENCE VOL AND REMOVAL OF THE REFERENCE FROM THE CONCATENATED ECC'D 5 VOL T2
for D in *;
do (cd $D && 
	fsl5.0-fslmerge -a t2avg_and_t2vols average5volt2_corr.nii.gz vol0000.nii.gz vol0001.nii.gz vol0002.nii.gz vol0003.nii.gz vol0004.nii.gz
	...
	rm -rf eccvol*.nii.gz
	rm -rf vol0000.nii.gz rm -rf vol0001.nii.gz rm -rf vol0002.nii.gz rm -rf vol0003.nii.gz rm -rf vol0004.nii.gz
); done
...
do (cd $D && 
	...
	avgvol=average5volt2_corr.nii.gz
...
do (cd $D && 
	...
	rm -rf average5volt2_corr.nii.gz 
